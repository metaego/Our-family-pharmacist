...
urlpatterns = [
    ...
    path('profile-id/surveys/survey-id/', views.recom_profile_total_report, name='profile_total_report'),
    ...
    path('profile-id/surveys/survey-id/rec-nut-products/nutri-num/', views.recom_products_nutri_base, name='products_nutri_base'),
    
    # AI추천받기: 영양제 추천 목록(영양 성분 리포트 기반)
    # menu: ai 영양제 추천받기(profile_info) > 영양 성분 리포트 > 영양제 추천 목록 
    # /recommends/{profile-id}/surveys/{survey-id}/rec-total-products/
    path('profile-id/surveys/survey-id/rec-total-products/', views.recom_products_profile_base, name='products_profile_base'),

    # AI추천받기: 영양제 추천 목록(나이 & 성별 기반)
    # menu: ai 영양제 추천받기(profile_info) > 영양 성분 리포트 > 영양제 추천 목록 
    # menu: home main(나이 & 성별 기반) > 영양제 추천 목록
    # /recommends/{profile-id}/surveys/{survey-id}/rec-collabo-products/
    path('profile-id/surveys/survey-id/rec-collabo-products/', views.recom_products_collabo_base, name='products_collabo_base')
]
