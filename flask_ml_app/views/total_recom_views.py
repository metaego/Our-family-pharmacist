...
def flask_ai_total_recom(survey_id):
    ...
    query = 'insert into recommendation (recommendation_id, created_at, custom_user_id, profile_id, survey_id) \
                values (NULL, %s, %s, %s, %s)'
    cursor = db.cursor()
    cursor.execute(query, (formatted_datetime, survey_data["custom_user_id"], survey_data["profile_id"], survey_data["survey_id"]))
    # db.commit()

    # recommendation pk 가져오기
    query = f'select recommendation_id from recommendation where survey_id = {survey_data["survey_id"]}'
    cursor = db.cursor(pymysql.cursors.DictCursor)
    cursor.execute(query)
    ...
    values = [(int(ingredient_id), recommendation_id["recommendation_id"]) for ingredient_id in recom_ingredient_id_list]
    ...
    for recom_product in recom_product_survey_list:
        query = "INSERT INTO recommendation_product (recommendation_product_id, product_id, recommendation_id) VALUES (NULL, %s, %s)"
        cursor.execute(query, (int(recom_product), recommendation_id["recommendation_id"]))
