...
@bp.route('/ai-sex-age-recom/<int:survey_id>/', methods=['POST'])
def flask_sex_age_base_ai_recom(survey_id):
    ...
    for code in list(survey_data["survey_function_code"].values()):
        if code in survey_df.columns:
            survey_df[code] = 1
    print(f'list(survey_data["survey_function_code"].values()): {list(survey_data["survey_function_code"].values())}')
